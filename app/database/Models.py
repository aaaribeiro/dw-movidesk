...
class Tickets(Base):
    ...
    __tablename__ = "mov_ent_tickets"

    ticket_id = Column(Integer, primary_key=True)
    organization_id = Column(String, ForeignKey("mov_ent_organizations.organization_id"))
    agent_id = Column(String, ForeignKey("mov_ent_agents.agent_id"))
...
class Organizations(Base):
    ...
    __tablename__ = "mov_ent_organizations"
...
class TimeAppointments(Base):
    ...
    __tablename__ = "mov_ent_timeappointment"

    time_appointment_id = Column(Integer, primary_key=True)
    ticket_id = Column(Integer, ForeignKey("mov_ent_tickets.ticket_id"), nullable=False)
    agent_id = Column(String, ForeignKey("mov_ent_agents.agent_id"), nullable=False)
    created_date = Column(DateTime)
...
class Users(Base):
    ...
    # relationships
    token = relationship("AccessToken", backref=backref("mov_auth_accesstoken", uselist=False))


class AccessToken(Base):

    __tablename__ = "mov_auth_accesstoken"
    
    token_id = Column(Integer, primary_key=True, nullable=False, autoincrement=True)
    user_id = Column(Integer, ForeignKey("mov_auth_users.user_id"), nullable=False)
